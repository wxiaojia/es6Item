...
var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); //接口模块：比如遗漏是服务端提供的或者自己计算的，交给前端
...
var Timer = function () {
	...
	_createClass(Timer, [{
		...
		value: function countdown(end, unpdate, handle) {
			...
			if (now - end) {
				...
				var px_d = 1000 * 60 * 60 * 60 * 24; //常量，一天的毫秒
				var px_h = 1000 * 60 * 60;
				var px_m = 1000 * 60;
				var px_s = 1000;
				//剩余多少天
				var d = Math.floor(last_time / px_d);
				var h = Math.floor((last_time - last_time / px_d) / px_h);
				var m = Math.floor((last_time - last_time / px_d - h * px_h) / px_m);
				var s = Math.floor((last_time - last_time / px_d - h * px_h - m * px_m) / px_s);
				var r = [];
				if (d > 0) {
					r.push('<em>' + d + '</em>\u5929');
				}
